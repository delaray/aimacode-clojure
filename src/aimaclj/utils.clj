...
;;;--------------------------------------------------------------------------
;;; A Clojure implenmentation of the utils.py file from
;;; https://github.com/aimacode/aima-python/blob/master/utils.py
;;;
;;; The basic approach will be to mimic the class & functions defined in
;;; utils.py since so many other modules use them.
;;;
;;; We will start will the actual python code from utils.py and incrementally
;;; delete it from this file as the Clojure implementation progresses.
;;;
;;; If some of the functionality provided by this file already exists we will
;;; use existing libraries rather than reinventing the wheel badly, matrix
...
;; count is a builtin Clojure function so bcount (boolean count) as name instead.

(defn bcount [seq]
  "Count the number of items in sequence that are interpreted as true."
  (count (filter #(if % true false) seq)))
...
(defn argmin [seq] (apply min seq))
(defn argmax [seq] (apply max seq))
